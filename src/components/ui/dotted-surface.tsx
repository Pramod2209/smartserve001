	...
	useEffect(() => {
		...
		for (let ix = 0; ix < AMOUNTX; ix++) {
			for (let iy = 0; iy < AMOUNTY; iy++) {
				...
				const colorX = ix / AMOUNTX;
				const colorY = iy / AMOUNTY;
				// Vibrant blue-purple-pink mix
				const r = 0.3 + 0.6 * colorX; 
				const g = 0.2 + 0.4 * colorY;
				const b = 0.8 + 0.2 * Math.sin(colorX * Math.PI);
				colors.push(r, g, b);
			}
		}
